                    ...
                    if(data.result)
                     {
                        ...
                        if(emoticon != null)
                        {
                            $mob.ext.ssdk_facebookmessengerShareGif(emoticon,function(data)
                            {
                        ...
                        else
                        {
                            ...
                            {
                                $mob.ext.ssdk_facebookmessengerShareImage(image,function(data)
                                {
                                    var resultData = data.result;
                                    if (data.state === $mob.shareSDK.responseState.Success)
                                    {
            ...
            if(data.result)
            {
                $mob.ext.isPluginRegisted("com.mob.sharesdk.connector.facebookmessenger", function (data){
                    if(data.result)
                     {
                        //有SDK
                        $mob.ext.ssdk_facebookmessengerShareAudio(audio,function(data)
                        {
                            var resultData = data.result;
                            if(data.state === $mob.shareSDK.responseState.Success)
                            {
            ...
            if(data.result)
            {
                $mob.ext.isPluginRegisted("com.mob.sharesdk.connector.facebookmessenger", function (data){
                    if(data.result)
                    {
                        ...
                        if (!/^(assets-library\:\/)?\//.test(video))
                        {
                            $mob.ext.ssdk_facebookmessengerShareVideo(video,function(data)
                            {
                                var resultData = data.result;
                                if(data.state === $mob.shareSDK.responseState.Success)
                                {
